...
//確認追蹤的PHP
    ...
    $em=$_SESSION["user"];
    $follow = "";   //哪位user要被追蹤
    if (isset($_GET["email"])) {  //有沒有按要追蹤誰
        $follow = $_GET["email"];
        if (!isset($_SESSION["user"][$follow])){ 
            $sql_query = "SELECT * FROM `followers` where email='$em' and fEmail='$follow'";
            $result = mysqli_query($db_link, $sql_query) or die("Query Fail! ".mysqli_error($db_link));            
            $numRow = mysqli_num_rows($result);
            if ($numRow != 0) {
                echo "<span style='color:darkblue; font-size: 20pt;'> Already followed.</span>";
            } else {
                $sql = "INSERT into `followers` (email, fEmail) values ('$em', '$follow')";  //加入資料表的指令
                $numRow = mysqli_num_rows($result);
                mysqli_query($db_link, $sql);
                echo "<span style='color:darkblue; font-size: 20pt;'> You're Following $follow Now!</span>";
            }
        }
    }
        ...
        <div class="container">
          <div class="row slider-text justify-content-center align-items-center">
            <div class="col-md-10 col-sm-12 ftco-animate mb-4 text-center">
              <p class="breadcrumbs"><span class="mr-2"><a href="index.php">Home</a></span> <span>Functions</span></p>
              <h1 class="mb-3 bread">Have A Crush!</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="container">
        <div class="row no-gutters">
          <div class="block-3 d-md-flex ftco-animate" align="center">
            <h2 class="mb-3">#Click pictures to follow them!</h2>
          </div>
          <?php
            #拿取資料PHP
            include("connDB.php");
            ...
            ?>
                ...
                <a href="portfolio.php?email=<?php echo $email;?>" class="image<?php echo $msg;?>" style="background-image: url('pictures/<?php echo $email;?>.jpg');"></a>
                  <div class="text">
          ...
          <!-- <div class="block-3 d-md-flex ftco-animate">
            <a href="portfolio-single.php" class="image order-2" style="background-image: url('images/work-2.jpg'); "></a>
            <div class="text order-1">
              <h4 class="subheading">Application</h4>
              <h2 class="heading">Even the all-powerful Pointing has no control</h2>
